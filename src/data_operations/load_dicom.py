...
from typing import Any, Union, Generator, Iterable
...
def _get_dicom_slices(dicom_path: str) -> Iterable[dicom.Dataset]:
    ...
    return tz.pipe(
        ...
        curried.filter(_dicom_type_is(g.CT_IMAGE)),
...
def _load_roi_name(
    ...
    name: str,
) -> Union[tuple[str, Generator[g.MaskType, None, None]], None]:
    """
...
def _load_rt_struct(dicom_path: str) -> Union[rt_utils.RTStructBuilder, None]:
    """
    ...
    return tz.pipe(
        ...
        list_files,
        curried.filter(
            lambda path: _dicom_type_is(g.RT_STRUCTURE_SET)(dicom.dcmread(path))
        ),
        list,
        apply_if_truthy_else_None(
...
def _preprocess_volume(
    ...
    return tz.pipe(
        ...
        map_interval(g.CT_RANGE, (0, 1)),  # TODO: What's the range???
    )


@curry
def _preprocess_mask(
    name_mask_pairs: tuple[str, g.MaskType], dicom_path: str
) -> Union[tuple[str, g.MaskType], None]:
...
@curry
def load_mask(dicom_path: str, preprocess: bool = True) -> Union[Mask, None]:
    """
